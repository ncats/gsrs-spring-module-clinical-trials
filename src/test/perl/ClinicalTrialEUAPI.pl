...

{
	...
	my $args = {
		...
		dump_response_content => 0, 	
		dump_response_perl_data => 0, 
		url => "$basePath/clinicaltrialeurope($trialNumber_ne)",
		condition => $condition,
		message => $message
	};
	test_get($args);
...
if (1) {
	...
	my $args = {
		...
		dump_response_content => 0, 	
		dump_response_perl_data => 0, 
		url => "$basePath/clinicaltrialeurope",
		condition => $condition,
		message => $message
	};
...
sub delete_helper { 
	...
	$client->addHeader('Accept', 'application/json');
	$client->addHeader('Content-Type', 'application/x-www-form-urlencoded');
	$client->DELETE("$basePath/clinicaltrialeurope($trialNumber)");
...
sub get_to_json { 
	...
	$client->addHeader('Accept', 'application/json');
	$client->addHeader('Content-Type', 'application/x-www-form-urlencoded');
	# print "URL: $basePath/clinicaltrialeurope($trialNumber)\n";
	$client->GET("$basePath/clinicaltrialeurope($trialNumber)");
